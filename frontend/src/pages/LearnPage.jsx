...
const ARTICLES = [
  ...
  { id: 5, cat: 'Science',   title: 'Understanding CO₂ equivalents and why they matter',               read: '7 min', date: 'May 5',  emoji: '🔬' },
...
const GLOSSARY_TERMS = [
  { term: 'Carbon Footprint',   def: 'The total amount of greenhouse gases produced to support human activities, expressed in equivalent tons of CO₂.',     related: ['CO₂ Equivalent', 'Scope 1'] },
  { term: 'CO₂ Equivalent',     def: 'A metric used to compare emissions from various greenhouse gases based on their global warming potential relative to CO₂.', related: ['Carbon Footprint'] },
  ...
  { term: 'Carbon Credit',      def: 'A permit allowing the holder to emit one tonne of CO₂ or equivalent greenhouse gas.', related: ['Carbon Offset'] },
  { term: 'Carbon Offset',      def: 'A reduction in emissions of CO₂ made to compensate for an emission made elsewhere.', related: ['Carbon Credit', 'Net Zero'] },
...
const MYTHS = [
  { myth: 'Recycling is the most impactful climate action I can take.', fact: 'Transport and diet have 10–50× more impact than recycling for most people. Recycling matters but it\'s not the top lever.', source: 'IPCC AR6, 2021' },
  { myth: 'Electric cars have a bigger carbon footprint than petrol cars due to battery production.', fact: 'Over its lifetime, an EV emits 50–70% less CO₂ than a petrol car — even accounting for battery manufacturing.', source: 'IEA, Global EV Outlook 2023' },
  ...
  { myth: 'Going vegetarian doesn\'t really help if I fly once a year.', fact: 'Both matter. A single transatlantic flight ≈ 1.5–3 tonnes CO₂. Giving up beef for a year ≈ 0.5–1 tonne. They compound.', source: 'BBC Climate Calculator, 2023' },
  ...
  { myth: 'Local food is always lower carbon than imported food.', fact: 'Transport accounts for <10% of most food\'s carbon footprint. What you eat matters far more than where it\'s from.', source: 'Our World in Data, 2020' },
];
    ...
    <div>
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-12">
          ...
          <p className="text-[#3e4a3d] font-semibold">No results for "{search}"</p>
          <button onClick={() => setSearch('')} className="text-[#006b2c] text-sm font-bold mt-2 hover:underline">Clear search</button>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {filtered.map((a) => (
            <article key={a.id} className="bg-white rounded-2xl p-5 border border-[#bdcaba]/30 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all cursor-pointer">
              <div className="w-12 h-12 bg-[#f1f3ff] rounded-xl flex items-center justify-center text-2xl mb-4">{a.emoji}</div>
...
function VideosTab() {
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
      {VIDEOS.map((v) => (
        <div key={v.id} className="bg-white rounded-2xl overflow-hidden shadow-sm border border-[#bdcaba]/30 hover:shadow-md transition-shadow cursor-pointer group">
...
function GlossaryTab() {
  ...
  return (
    <div>
      <div className="relative mb-5">
        <MaterialIcon name="search" className="absolute left-4 top-1/2 -translate-y-1/2 text-[#bdcaba] text-xl" />
        <input type="search" placeholder="Search glossary…" value={search}
          onChange={(e) => setSearch(e.target.value)}
          className="w-full pl-12 pr-4 py-3 bg-white rounded-xl border border-[#bdcaba] focus:ring-2 focus:ring-[#006b2c] outline-none text-[#141b2b] placeholder:text-[#bdcaba]" />
      </div>
            ...
            {filtered.filter((t) => t.term[0] === l).map((t) => (
              <div key={t.term} className="bg-white rounded-xl p-5 border-l-4 border-[#006b2c] shadow-sm mb-3">
                ...
                <div className="flex flex-wrap gap-2">
                  {t.related.map((r) => (
                    <span key={r} className="text-[10px] font-bold bg-[#f1f3ff] text-[#006b2c] px-2 py-0.5 rounded-full cursor-pointer hover:bg-[#b1f2be] transition-colors">
                      {r}
                    </span>
                  ))}
                </div>
...
// =============================================================================
// SECTION: MythVsFactTab — flip cards
// =============================================================================
function MythVsFactTab() {
  const [flipped, setFlipped] = useState([]);
  const toggle = (id) => setFlipped((prev) => prev.includes(id) ? prev.filter((i) => i !== id) : [...prev, id]);

  return (
    <div>
      <p className="text-sm text-[#3e4a3d] mb-6">
        Click any card to reveal the fact. These are some of the most common climate misconceptions.
      </p>
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
        {MYTHS.map((m, i) => {
              ...
              {!isFlipped ? (
                <>
                  <div>
                    ...
                    <p className="text-sm font-semibold text-[#141b2b] italic leading-snug">"{m.myth}"</p>
                  </div>
                  <p className="text-[11px] text-[#bdcaba] mt-3 font-bold">Click to reveal the fact →</p>
                </>
              ) : (
                <>
                  <div>
...
export default function LearnPage() {
  ...
  return (
    <DashboardShell>
      ...
      {/* Tab bar */}
      <div className="flex flex-wrap gap-1 bg-[#f1f3ff] p-1 rounded-2xl mb-6 w-full sm:w-fit"
        role="tablist" aria-label="Learn sections">
        {TABS.map((t) => (
          <button
            key={t.id}
            role="tab"
